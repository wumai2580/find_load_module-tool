...
#ifdef _WIN32
static std::vector<char> read_file_buf_w(const std::wstring& file_path) {
	std::vector<char> buffer;
	FILE* f = _wfopen(file_path.c_str(), L"rb");
	if (!f) return buffer;
	if (fseek(f, 0, SEEK_END) != 0) { fclose(f); return buffer; }
	long size = ftell(f);
	if (size <= 0) { fclose(f); return buffer; }
	rewind(f);
	buffer.resize(static_cast<size_t>(size));
	size_t readn = fread(buffer.data(), 1, static_cast<size_t>(size), f);
	(void)readn;
	fclose(f);
	return buffer;
}
...
static bool write_file_bytes(const char* file_path, long offset, const char* bytes, size_t len) {
...
static size_t get_input_hex_number() {
	std::string input;
	std::cin >> input;
